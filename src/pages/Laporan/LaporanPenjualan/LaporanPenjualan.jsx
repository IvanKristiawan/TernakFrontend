...
const LaporanPenjualan = () => {
  ...
  if (grouping === "STOK") {
    for (let i = 0; i < Object.keys(lapPenjualansData).length; i++) {
      list.push(
        <tr style={tableText}>
          <td style={{ fontWeight: 700 }} colspan="3">
            {lapPenjualansData[keys[i]][0].groupStok} :
          </td>
        </tr>
      );
      let groupBy = lapPenjualansData[keys[i]].reduce((group, pengajuan) => {
      ...
      for (let j = 0; j < Object.keys(keysSubGroup).length; j++) {
        for (let k = 0; k < groupBy[keysSubGroup[j]].length; k++) {
          totalQty += groupBy[keysSubGroup[j]][k].qtyJualChild;
          totalHarga += groupBy[keysSubGroup[j]][k].hargaJualChild;
          totalSubTotal += groupBy[keysSubGroup[j]][k].subtotalJualChild;
          list.push(
            <tr style={tableText}>
              <td style={{ paddingLeft: "15px" }}>
                {groupBy[keysSubGroup[j]][k].jual.noNotaJual}
              </td>
              <td>{groupBy[keysSubGroup[j]][k].tanggalJual}</td>
  ...
  return (
    <Container>
      <h3>Laporan</h3>
      <h5 style={{ fontWeight: 400 }}>Laporan Penjualan</h5>
      <hr />
      <Box sx={spacingTop}>
